# Practical 2: Binomial and Poisson Distributions
...
*Q35. Copy down the binomial probability mass function (from Chapter* \@ref(appendix)*) and use it as an alternative method for calculating the fertility proportions for two eggs (a, b, c above).*
...
The Poisson formula is simple - copy it from Chapter \@ref(appendix) into here: \newline \newline \newline \newline \newline \newline \newline $\bar{y}$ is the mean, $x$ is the outcome of interest, $e$ is Euler's number, and $!$ is factorial.
...
Consider randomly throwing a 1$m^2$ quadrat repeatedly on a sandy beach covered in worm casts (Fig. \@ref(fig:worm-casts)). You then count the number of casts in each quadrat.
...
Generate some random numbers from the distributions in Fig \@ref(fig:binom-pois-approx) and calculate their mean and variance. Here is $y \sim Pois(\lambda=5)$:
