# Practical 4: T distribution and confidence intervals
...
```{r t-stat-hist, fig.cap="Histogram of 100,000 t-statistics calculated from 100,000 samples, along with the corresponding theoretical t-distribution (solid line) and a standard normal (dotted line).", out.width="70%", out.height="70%", fig.dim=c(6,4)}
...
```{r ci-examp3, fig.cap="Histogram illustrating the barnacle population with population mean (blue) and sample means with 95\\% CIs (red) repeated across 5 samples.", fig.dim=c(5,4)}
num_samples <- 5
sample_size <- 3
